...
DB_DIR = os.getenv("DB_DIR", BASE_DIR)  # if you ever see 'readonly', set DB_DIR=/tmp in Render env
os.makedirs(DB_DIR, exist_ok=True)
DB_PATH = os.path.join(DB_DIR, "analytics.db")

def _db():
    # isolation_level=None = autocommit; WAL reduces writer blocking for >1 worker if you ever enable it
...
# --------- Lightweight spaced resurfacing ----------
...
def upsert_spaced_tag(tag: str, success: bool):
    try:
        ...
        if not row:
            ...
            conn.execute(
                "INSERT INTO spaced (session_id,tag,interval_idx,next_due_ts,last_result,created_ts,updated_ts) VALUES (?,?,?,?,?,?,?)",
            ...
            next_due = _days_from_now(INTERVALS_DAYS[idx])
            conn.execute(
                "UPDATE spaced SET interval_idx=?, next_due_ts=?, last_result=?, updated_ts=? WHERE id=?",
...
def queued_spaced_count():
    try:
        conn = _db()
        cur = conn.execute("SELECT COUNT(*) FROM spaced WHERE session_id=? AND next_due_ts<=?",
                           (session.get("sid"), _now_ts()))
        n = cur.fetchone()[0]
        conn.close()
        return n
    except Exception as e:
...
# ======================= Scoring / XP policy =======================
XP = {
    "miss_required_stage_cap": 60,    # miss required in any section → cap
    "contra_malus_heavy": 20,         # clearly unsafe
    "contra_malus_moderate": 12,      # dubious/unsafe
...
.overlay{position:absolute;inset:0;pointer-events:none;opacity:0;transition:opacity .2s}
.overlay.show{opacity:.85}
...
<body class="min-h-screen bg-gradient-to-br from-sky-500 via-indigo-500 to-emerald-500 text-white p-4">
  <div class="w-full max-w-4xl mx-auto bg-white/15 backdrop-blur-md rounded-2xl p-6 shadow-xl">
    ...
    <div class="flex flex-wrap gap-2 my-4">
      ...
      <span class="px-3 py-1 rounded-full {% if due_count>0 %}bg-rose-400/80{% else %}bg-white/20{% endif %}">🧩 Weak spots due: {{ due_count }}</span>
    </div>
  ...
  <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow p-5">
    ...
    <div class="grid gap-4">
      ...
      <div class="bg-slate-50 border rounded-xl p-4">
        ...
        <ul class="list-disc ml-6">{% for o in curriculum_outcomes %}<li>{{ o }}</li>{% endfor %}</ul>
      </div>

      <div class="bg-slate-50 border rounded-xl p-4">
        <h3 class="font-bold">AUS escalation cues — when to call the registrar</h3>
        <ul class="list-disc ml-6">{% for e in escalation_cues %}<li>{{ e }}</li>{% endfor %}</ul>
      </div>
...
def _checklist(name, items, prev):
    ...
    for it in items:
        ...
        html += f"""<label class="block bg-slate-800 p-3 rounded-lg mb-2">
...
def start_case():
    # In future: use posted "block" to switch cases. For now Cardiology only.
...
def _render_stage(state):
    ...
    if key == "presenting":
        ...
        body = f"<p class='mb-2'>{ecg['prompt']}</p>" + _checklist("ecg_tick", ecg["checklist"], prev) + "<div class='text-xs opacity-70 mt-2'>We score by required picks chosen and contraindicated picks avoided.</div>"

    elif key == "order_set":
        items = CASE["order_set"]["items"]
        prev = state["decisions"].get("order_set",{}).get("ticks",[])
        body = f"<p class='mb-2'>{CASE['order_set']['prompt']}</p>" + _checklist("order_tick", items, prev)

    elif key == "labs_reasoning":
        lr = CASE["labs_reasoning"]
        prev = state["decisions"].get("labs_reasoning",{}).get("ticks",[])
        body = "<h3 class='font-bold'>Key labs</h3>" + _table_html(lr["table"]) + f"<p class='mt-3 mb-2'>{lr['question']}</p>" + _checklist("labs_tick", lr["options"], prev)

    elif key == "imaging_panel":
        ip = CASE["imaging_panel"]
        prev = state["decisions"].get("imaging_panel",{}).get("ticks",[])
        body = f"<p class='mb-2 whitespace-pre-line'>{ip['prompt']}</p>" + _checklist("img_tick", ip["checklist"], prev)

    elif key == "plan_builder":
        items = CASE["plan_builder"]["items"]
        prev = state["decisions"].get("plan_builder",{}).get("ticks",[])
        body = f"<p class='mb-2'>{CASE['plan_builder']['prompt']}</p>" + _checklist("plan_tick", items, prev)

    elif key == "handoff":
        items = CASE["handoff"]["items"]
        prev = state["decisions"].get("handoff",{}).get("ticks",[])
...
def stage():
    ...
    if request.method == "POST":
        ...
        if key == "immediate_actions":
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["immediate_actions"]["items"], XP["immediate_actions_full"],
                "Immediate Actions", state, review_tag_ok="ACS_ECG_10MIN"
            )
            state["score"] += pts; state["xp_earned"] += pts
            state["xp_lines"].append(f"+{pts} Immediate actions")
            upsert_spaced_tag("ACS_MONITOR_IV", not missing)
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["targeted_history"]["items"], XP["history_select_full"],
                "Targeted History", state, review_tag_ok="HIST_RED_FLAGS"
            )
            state["score"] += pts; state["xp_earned"] += pts
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["focused_exam"]["items"], XP["focused_exam_full"],
                "Focused Exam", state, review_tag_ok="EXAM_OBS"
            )
            state["score"] += pts; state["xp_earned"] += pts
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["order_set"]["items"], XP["order_set_full"],
                "Order-Set", state, review_tag_ok="ACS_PATHWAY_BUNDLE"
            )
            state["score"] += pts; state["xp_earned"] += pts
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["plan_builder"]["items"], XP["plan_builder_full"],
                "Plan Builder", state, review_tag_ok="ACS_PATHWAY"
            )
            state["score"] += pts; state["xp_earned"] += pts
            ...
            pts, missing, unsafe = _score_required_contra(
                ticks, CASE["handoff"]["items"], XP["handoff_full"],
                "Handoff", state, review_tag_ok="HANDOFF_DISCIPLINE"
            )
            state["score"] += pts; state["xp_earned"] += pts
    ...
    return render_template_string(
        ...
        stage_num=state["stage_idx"]+1,
        stage_total=len(state["flow"]),
        stage_label=_stage_name(key),
        vitals=state["vitals"],
        body=body,
        show_imaging=(key == "imaging_panel")
    )
...
def feedback():
    ...
    # Immediate Actions
    imm_ticks = d.get("immediate_actions",{}).get("ticks",[])
    I = _map(CASE["immediate_actions"]["items"])
    _mk_section(
        "Immediate Actions",
        ", ".join([I[i]["text"] for i in imm_ticks]) or "None",
        [i["text"] for i in CASE["immediate_actions"]["items"] if i.get("required") and i["id"] not in imm_ticks],
        [I[i]["text"] for i in imm_ticks if I.get(i,{}).get("contra")],
        "ECG ≤10 min and monitor/IV are non-negotiable. Waiting for troponin or discharging now is unsafe in typical ACS."
    )

    # Targeted History
    th_ticks = d.get("targeted_history",{}).get("ticks",[])
    H = _map(CASE["targeted_history"]["items"])
    _mk_section(
        "Targeted History",
        ", ".join([H[i]["text"] for i in th_ticks]) or "None",
        [i["text"] for i in CASE["targeted_history"]["items"] if i.get("required") and i["id"] not in th_ticks],
        [],
        "Lead with red flags and pain character; keep it tight."
    )

    # Focused Exam
    fx_ticks = d.get("focused_exam",{}).get("ticks",[])
    F = _map(CASE["focused_exam"]["items"])
    _mk_section(
        "Focused Exam",
        ", ".join([F[i]["text"] for i in fx_ticks]) or "None",
        [i["text"] for i in CASE["focused_exam"]["items"] if i.get("required") and i["id"] not in fx_ticks],
        [],
        "Trend vitals and listen to lungs/heart immediately."
    )

    # ECG
    er_ticks = d.get("ecg_read",{}).get("ticks",[])
    E = _map(CASE["ecg_read"]["checklist"])
    _mk_section(
        "ECG Checklist",
        ", ".join([E[i]["text"] for i in er_ticks]) or "None",
        [E[i]["text"] for i in E if E[i].get("required") and i not in er_ticks],
        [E[i]["text"] for i in er_ticks if E[i].get("contra")],
        "Explicitly call out rate, rhythm, ST/T changes; here ST depression V4–V6 is high risk."
    )

    # Order-set
    os_ticks = d.get("order_set",{}).get("ticks",[])
    O = _map(CASE["order_set"]["items"])
    _mk_section(
        "Initial Order-Set",
        ", ".join([O[i]["text"] for i in os_ticks]) or "None",
        [O[i]["text"] for i in O if O[i].get("required") and i not in os_ticks],
        [O[i]["text"] for i in os_ticks if O[i].get("contra")],
        "Start aspirin early (if no CI), consider GTN if BP adequate, telemetry, CXR. No blanket thrombolysis."
    )

    # Labs
    lb_ticks = d.get("labs_reasoning",{}).get("ticks",[])
    L = {o["id"]: o for o in CASE["labs_reasoning"]["options"]}
    _mk_section(
        "Labs Reasoning",
        ", ".join([L[i]["text"] for i in lb_ticks]) or "None",
        [L[i]["text"] for i in L if L[i].get("correct") and i not in lb_ticks],
        [L[i]["text"] for i in lb_ticks if L[i].get("contra")],
        "Baseline hs-trop doesn’t exclude ACS — trend serials. Renal function supports safe contrast if needed."
    )

    # Imaging
    im_ticks = d.get("imaging_panel",{}).get("ticks",[])
    M = _map(CASE["imaging_panel"]["checklist"])
    _mk_section(
        "Imaging (CXR)",
        ", ".join([M[i]["text"] for i in im_ticks]) or "None",
        [M[i]["text"] for i in M if M[i].get("required") and i not in im_ticks],
        [M[i]["text"] for i in im_ticks if M[i].get("contra")],
        "Read systematically; exclude widened mediastinum or large PTX; note congestion."
    )

    # Plan
    pl_ticks = d.get("plan_builder",{}).get("ticks",[])
    P = _map(CASE["plan_builder"]["items"])
    _mk_section(
        "Plan",
        ", ".join([P[i]["text"] for i in pl_ticks]) or "None",
        [P[i]["text"] for i in P if P[i].get("required") and i not in pl_ticks],
        [P[i]["text"] for i in pl_ticks if P[i].get("contra")],
        "Enter pathway, serial trops, escalate to cardiology with dynamic ECG or ongoing pain."
    )

    # Handoff
    ho_ticks = d.get("handoff",{}).get("ticks",[])
    H2 = _map(CASE["handoff"]["items"])
    _mk_section(
        "Registrar Handoff",
        ", ".join([H2[i]["text"] for i in ho_ticks]) or "None",
        [H2[i]["text"] for i in H2 if H2[i].get("required") and i not in ho_ticks],
        [H2[i]["text"] for i in ho_ticks if H2[i].get("contra")],
